...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const Ambulance_Edit_Profile(title: 'SIGNUP'),
    );
  }
}

class Ambulance_Edit_Profile extends StatefulWidget {
  const Ambulance_Edit_Profile({super.key, required this.title});
...
class _Ambulance_Edit_ProfileState extends State<Ambulance_Edit_Profile> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (status == "ok") {
          ...
          String photo_ = sh.getString("imageurl").toString() +
              jsonDecode(response.body)['photo'];
          String proof_ = sh.getString("imageurl").toString() +
              jsonDecode(response.body)['proof'];
          String vphoto_ = sh.getString("imageurl").toString() +
  ...
  void senddata() async {
    ...
    String url = sh.getString("url").toString();
    String lid=sh.getString("lid").toString();
    final urls = Uri.parse('$url/ambulance_edit_profile_post/');
    try {
