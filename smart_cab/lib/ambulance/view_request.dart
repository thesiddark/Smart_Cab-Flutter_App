...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const AmbulanceRequestStatus(title: ''),
    );
  }
}

class AmbulanceRequestStatus extends StatefulWidget {
  const AmbulanceRequestStatus({super.key, required this.title});

  final String title;

  @override
  State<AmbulanceRequestStatus> createState() => _AmbulanceRequestStatusState();
}

class _AmbulanceRequestStatusState extends State<AmbulanceRequestStatus> {
  _AmbulanceRequestStatusState() {
    viewmanager();
  }
  ...
  Future<void> viewmanager() async {
    ...
    try {
      ...
      String url = '$urls/ambulance_view_request/';
          ...
          appBar: AppBar(
            leading: BackButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => AmbulanceHome(title: '')),
                );
              },
            ),
                        ...
                        children: [
                          SizedBox(width: 5,),ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                final urls = Uri.parse('$url/ambulance_approve_request/');
                                try {
                          ...
                          ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                String rid = id_[index];
                                String url = sh.getString('url').toString();
                                final urls = Uri.parse('$url/ambulance_reject_request/');
                                try {
                                  final response = await http.post(urls, body: {'rid': rid});
                                  if (response.statusCode == 200) {
                                    String status = jsonDecode(response.body)['status'];
