...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const AmbulanceApprovedStatus(title: ''),
    );
  }
}

class AmbulanceApprovedStatus extends StatefulWidget {
  const AmbulanceApprovedStatus({super.key, required this.title});

  final String title;

  @override
  State<AmbulanceApprovedStatus> createState() => _AmbulanceApprovedStatusState();
}

class _AmbulanceApprovedStatusState extends State<AmbulanceApprovedStatus> {
  _AmbulanceApprovedStatusState() {
    viewmanager();
  }

  List<String> id_ = <String>[];
  List<String> uid_ = <String>[];
  ...
  Future<void> viewmanager() async {
    ...
    try {
      ...
      String url = '$urls/ambulance_view_approved_request/';
                        ...
                        child: IconButton(
                          ...
                          onPressed: () async {
                            SharedPreferences sh = await SharedPreferences
                                .getInstance();
                            String mid = uid_[index].toString();
                            String url = sh.getString('url').toString();
                            final urls = Uri.parse(
                                '$url/ambulance_get_user_location/');
                            try {
