...
import 'package:smart_cab/user/payment.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const DriverRequestStatus(title: ''),
    );
  }
}

class DriverRequestStatus extends StatefulWidget {
  const DriverRequestStatus({super.key, required this.title});

  final String title;

  @override
  State<DriverRequestStatus> createState() => _DriverRequestStatusState();
}

class _DriverRequestStatusState extends State<DriverRequestStatus> {
  _DriverRequestStatusState() {
    viewmanager();
  }
  ...
  Future<void> viewmanager() async {
    ...
    try {
      ...
      String url = '$urls/user_view_request_status/';
      ...
      for (int i = 0; i < arr.length; i++) {
        ...
        paid.add(arr[i]['paid']);
        selfno.add(arr[i]['selfno'].toString());
        driverno.add(arr[i]['driverno'].toString());
      }
                                ...
                                onPressed: () async {
                                  if (paid_[index] != 'yes') {
                                    ...
                                    Navigator.push(context, MaterialPageRoute(
                                      builder: (context) => CabPayment(),));
                                    _sendSMS([selfno_[index],driverno_[index]]);
                                  }
                                },
                                child: paid_[index] == 'yes'
                                    ? Text('paid')
                                    : Text("Payment"),
                              ),
                            ),
                            ...
                            Visibility(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () async {
                                  SharedPreferences sh = await SharedPreferences
                                      .getInstance();
                                  String mid = did_[index].toString();
                                  String url = sh.getString('url').toString();
                                  final urls = Uri.parse(
                                      '$url/user_get_driver_location/');
                                  try {
